...
export default function BuyOriginal() {
  ...
  const filteredEvents = search
    ? sampleOriginalEvents.filter(e =>
        e.name.toLowerCase().includes(search.toLowerCase()) ||
        e.venue.toLowerCase().includes(search.toLowerCase())
      )
    : sampleOriginalEvents;
  ...
  return (
    <div className="flex items-center justify-center h-full">
      <div className="bg-white p-8 rounded-lg shadow-lg text-center w-full max-w-2xl">
        <h1 className="text-3xl font-bold text-[#A31D1D] mb-4 font-chonburi">Buy Original Tickets</h1>
        <p className="text-[#A31D1D] font-domine mb-6 text-base">
          Here are original tickets directly from the event organizer.
        </p>
        {showWalletMessage && (
        ...
        <div className="flex justify-center mb-6">
          <input
            ...
            placeholder="Search for original tickets..."
            value={search}
